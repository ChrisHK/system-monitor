...
router.post('/', auth, checkRole(['admin']), catchAsync(async (req, res) => {
...
router.get('/', auth, checkRole(['admin']), catchAsync(async (req, res) => {
    const result = await pool.query(`
        SELECT 
            u.id, 
            u.username, 
...
router.put('/:id', auth, checkRole(['admin']), catchAsync(async (req, res) => {
    const { id } = req.params;
    const { username, role, group_id } = req.body;

    if (!username || !role) {
        throw new ValidationError('Username and role are required');
    }
